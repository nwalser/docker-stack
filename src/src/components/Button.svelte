<script lang="ts">
	...
	export let direction: ArrowDirection = ArrowDirection.None;
</script>

<a {href} class="inline-flex gap-0.5 justify-center overflow-hidden text-sm font-medium transition {variantStyles[variant]}">
	{#if direction == ArrowDirection.Left}
		<svg viewBox="0 0 20 20" fill="none" aria-hidden="true" class="mt-0.5 h-5 w-5 -ml-1 rotate-180">
			<path stroke="currentColor" d="m11.5 6.5 3 3.5m0 0-3 3.5m3-3.5h-9" />
		</svg>
	{/if}
